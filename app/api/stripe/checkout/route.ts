...
function getBaseUrl() {
  // preferisci NEXT_PUBLIC_SITE_URL, fallback su Vercel URL se esiste
...
export async function POST(req: Request) {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      mode: "payment",
      line_items: [{ price: book.stripePriceId, quantity: 1 }],

      // ✅ globali
      success_url: `${baseUrl}/carrello/success?session_id={CHECKOUT_SESSION_ID}`,
      cancel_url: `${baseUrl}/carrello`,

      metadata: { kind: "single", slug: book.slug, source: "editoria" },
    });
