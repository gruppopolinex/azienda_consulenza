...
const SICUREZZA_CASES: SicurezzaCase[] = [
  {
  ...
  },
  {
    ...
    title: "DVR e procedure per impianto di trattamento",
    location: "Treviso (TV)",
...
// Filtra solo i bandi relativi alla sicurezza
const SICUREZZA_GRANTS: Grant[] = GRANTS.filter((g) => {
  ...
  const byArea =
    ...
    anyG.areas.some(
      (a: string) => a.toLowerCase() === "sicurezza".toLowerCase()
    );

  const byLegacy =
    anyG.ambito === "Sicurezza" ||
    anyG.settore === "Sicurezza" ||
    (Array.isArray(anyG.tags) &&
      anyG.tags.some((t: string) => t.toLowerCase().includes("sicurezza")));
  ...
  if (!items.length) {
    return (
      <div className="mt-6 rounded-2xl border border-slate-200 bg-slate-50 p-6 text-sm text-slate-600">
        Stiamo selezionando i bandi più rilevanti per l&apos;ambito sicurezza.
