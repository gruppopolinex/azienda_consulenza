...
// Progetti sicurezza dal portfolio
const SICUREZZA_CASES: SicurezzaCase[] = PROJECTS.filter(
  (p) => p.category === "Sicurezza"
).map((p: Project) => ({
  slug: p.slug,
  title: p.title,
  location: p.location,
...
// Bandi sicurezza (nuovo campo aree)
const SICUREZZA_GRANTS: Grant[] = GRANTS.filter((g) =>
  g.aree?.includes("Sicurezza")
);
...
export default function SicurezzaPage() {
  const hasGrants = SICUREZZA_GRANTS.length > 0;
  const hasCourses = SICUREZZA_COURSES.length > 0;
  const hasBooks = SICUREZZA_BOOKS.length > 0;
  const hasCases = SICUREZZA_CASES.length > 0;
    ...
    <>
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-2 sm:py-3">
        ...
        <section className="mt-10">
          <SectionHeader title="Come lavoriamo sui progetti di sicurezza" />
        ...
        {/* BANDI / FINANZA AGEVOLATA PER LA SICUREZZA */}
        {hasGrants && (
          <section className="mt-10">
        ...
        {hasCourses && (
          <section className="mt-10">
            <SectionHeader title="Formazione tecnica in ambito sicurezza" />
            <SicurezzaFormazioneCarousel items={SICUREZZA_COURSES} />
        ...
        {/* EDITORIA AREA SICUREZZA */}
        {hasBooks && (
          <section className="mt-10">
            <SectionHeader title="Manuali e pubblicazioni in ambito sicurezza" />
            <SicurezzaEditoriaCarousel items={SICUREZZA_BOOKS} />
...
/* ====== Carosello casi studio sicurezza ====== */

function SicurezzaCasesCarousel({ items }: { items: SicurezzaCase[] }) {
...
function SicurezzaCaseCard({ c }: { c: SicurezzaCase }) {
  return (
...
/* ====== Carosello bandi sicurezza ====== */

function SicurezzaGrantsCarousel({ items }: { items: Grant[] }) {
  if (!items.length) return null;
      ...
      <div className="flex flex-col h-full">
        ...
        <div className="mt-4 flex items-center justify-between">
          <span className="inline-flex items-center rounded-full bg-emerald-50 text-emerald-700 ring-1 ring-emerald-100 px-3 py-1 text-[11px] font-semibold">
            Bando per interventi sulla sicurezza
          </span>
...
/* ====== Carosello FORMAZIONE sicurezza ====== */

function SicurezzaFormazioneCarousel({ items }: { items: Course[] }) {
  if (!items.length) return null;
...
/* ====== Carosello EDITORIA sicurezza ====== */

function SicurezzaEditoriaCarousel({ items }: { items: Book[] }) {
  if (!items.length) return null;
